...
export async function erpRoutes(app: FastifyInstance) {
  ...
  app.post("/erp/connect", async (req: FastifyRequest<{ Body: { account_id: string, type: 'siigo' | 'erp', api_key?: string, username?: string, password?: string } }>, reply: FastifyReply) => {
    try {
  ...
  app.post("/erp/sync/products", async (req: FastifyRequest<{ Body: { account_id: string, channel_id: string } }>, reply: FastifyReply) => {
    try {
      getUserFromRequest(req);
  ...
  app.post("/erp/sync/inventory", async (req: FastifyRequest<{ Body: { account_id: string, channel_id: string } }>, reply: FastifyReply) => {
    try {
      getUserFromRequest(req);
...
async function testSiigoConnection(apiKey: string) {
  try {
...
async function syncERPProducts(channel: any) {
  try {
...
async function syncERPInventory(channel: any) {
  try {
