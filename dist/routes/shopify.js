...
exports.shopifyRoutes = shopifyRoutes;
...
async function shopifyRoutes(app) {
    ...
    app.get("/shopify/callback", async (req, reply) => {
        try {
            ...
            const { data: nonceRecord } = await supabaseClient_1.supabase
                .from("sync_logs")
                .select("*")
...
async function processShopifyOrder(channel, orderData) {
    try {
        ...
        const { data: existingOrder } = await supabaseClient_1.supabase
            .from("orders")
            .select("id")
        ...
        // Create order
        const { data: newOrder, error: orderError } = await supabaseClient_1.supabase
            .from("orders")
            .insert([order])
            .select()
            .single();
...
async function processShopifyProduct(channel, productData) {
    try {
...
async function processShopifyInventory(channel, inventoryData) {
    try {
